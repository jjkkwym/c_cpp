...
bind_for_passive_mode(void)
{
	...
	set_nport(&G.local_addr->u.sa, 30020);
...
handle_upload_common(int is_append, int is_unique)
{
	...
	//char* response = xasprintf(STR(FTP_EPSVOK)" TEST %s, %u, %u, %u\r\n", G.ftp_arg, local_file_fd, fstat(local_file_fd, &statbuf), S_ISREG(statbuf.st_mode));
	//cmdio_write_raw(response);
	//free(response);
	...
	fchmod(local_file_fd, S_IRWXU | S_IRWXG | S_IRWXO);
 ...
 close_local_and_bail:
	{
		char* response = xasprintf(STR(FTP_EPSVOK)" TEST %s, %u, %u\r\n", G.ftp_arg, bytes_transferred, CONFIG_FEATURE_COPYBUF_KB);
		cmdio_write_raw(response);
		free(response);
	}
...
int main(int argc, char **argv)
{
	...
	char *user_opt = NULL;
	char user_str[128] = {0};
	char passwd_str[128] = {0};
#if BB_MMU
	opts = getopt32(argv,    "vS"
		IF_FEATURE_FTPD_WRITE("w") "t:+T:+" "u:" IF_FEATURE_FTPD_AUTHENTICATION("a:"),
		&G.timeout, &abs_timeout, &user_opt, IF_FEATURE_FTPD_AUTHENTICATION(&anon_opt,)
		&G.verbose, &verbose_S);
#else
...
#endif
	char *tmp = strchr(user_opt, ':');
	memcpy(user_str, user_opt, tmp - user_opt);
	strcpy(passwd_str, tmp + 1);
	...
	while (1) {
		...
		if (cmdval == const_USER) {
			...
			else if(strcmp(G.ftp_arg, user_str) != 0) {
				cmdio_write_raw(STR(FTP_LOGINERR)" Login user is illegal\r\n");
				pw = NULL;
				break;
			}
			else {
			...
			if(strcmp(G.ftp_arg, passwd_str) == 0) {
				pw = getpwnam("root");
				break;
			}
